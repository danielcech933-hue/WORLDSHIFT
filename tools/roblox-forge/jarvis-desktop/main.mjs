...
function startNode(script, extraEnv={}){const env={...process.env,FORGE_PROJECT_ROOT:projectRoot,JARVIS_PORT:String(PORT),JARVIS_WORKER_PORT:String(WORKER_PORT),JARVIS_POLICY:process.env.JARVIS_POLICY||'supervised',...extraEnv};const child=spawn(process.execPath,[script],{cwd:projectRoot,env,windowsHide:true,shell:false,stdio:['ignore','pipe','pipe']});child.stdout.on('data',d=>console.log(`[JARVIS] ${String(d).trimEnd()}`));child.stderr.on('data',d=>console.error(`[JARVIS] ${String(d).trimEnd()}`));return child}
...
async function ensureRuntime(){if(!projectRoot)throw new Error('WORLDSHIFT project was not found.');if(!backend||backend.exitCode!==undefined){backend=startNode(path.join(projectRoot,'tools','roblox-forge','jarvis-backend-v2.mjs'),{JARVIS_MODEL:process.env.JARVIS_MODEL||'qwen2.5-coder:3b',JARVIS_AUTONOMY:'0'});}const backendOk=await waitHttp(`http://127.0.0.1:${PORT}/health`);if(!backendOk)throw new Error('JARVIS backend did not become ready. Is Ollama running?');const workerOk=Boolean(worker&&worker.exitCode===undefined)&&await waitHttp(`http://127.0.0.1:${WORKER_PORT}/health`,1000);return{backend:backendOk,worker:workerOk,projectRoot}}
...
ipcMain.handle('close-window',()=>{windowRef?.close();return true});
ipcMain.handle('minimize-window',()=>{windowRef?.minimize();return true});
ipcMain.handle('toggle-always-on-top',()=>{if(windowRef)windowRef.setAlwaysOnTop(!windowRef.isAlwaysOnTop());return windowRef?.isAlwaysOnTop()??false});
ipcMain.handle('drag-window',(_event,{x,y})=>{if(windowRef)windowRef.setPosition(Math.round(x),Math.round(y));return true});
ipcMain.handle('jarvis-request',async(_event,{path:requestPath,body:requestBody})=>{const clean=String(requestPath||'');if(!/^\/(health|state|chat|autonomy|pc|tasks)(\/.*)?$/.test(clean))throw new Error('Unsupported JARVIS endpoint.');const options={method:requestBody===undefined?'GET':'POST',headers:{'Content-Type':'application/json'}};if(requestBody!==undefined)options.body=JSON.stringify(requestBody);const response=await fetch(`http://127.0.0.1:${PORT}${clean}`,options);const text=await response.text();let data;try{data=JSON.parse(text)}catch{data={raw:text}};if(!response.ok)throw new Error(data.error||`JARVIS HTTP ${response.status}`);return data});
async function createWindow(){windowRef=new BrowserWindow({width:480,height:650,minWidth:430,minHeight:560,show:false,frame:false,transparent:true,hasShadow:true,resizable:true,alwaysOnTop:true,title:'JARVIS',backgroundColor:'#00000000',webPreferences:{preload:path.join(DESKTOP_DIR,'preload.cjs'),contextIsolation:true,nodeIntegration:false,sandbox:true}});windowRef.setMenuBarVisibility(false);windowRef.on('ready-to-show',()=>{const display=windowRef.getPrimaryDisplay();const area=display.workArea;windowRef.setPosition(area.x+area.width-500,area.y+area.height-690);windowRef.show();});await windowRef.loadFile(path.join(DESKTOP_DIR,'index.html'));windowRef.webContents.setWindowOpenHandler(({url})=>{shell.openExternal(url);return{action:'deny'}})}
...
app.on('before-quit',stopRuntime);app.on('window-all-closed',()=>{if(process.platform!=='darwin')app.quit()});
