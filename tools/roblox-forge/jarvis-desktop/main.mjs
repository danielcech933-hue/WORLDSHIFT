...
ipcMain.handle('project-info',()=>({projectRoot}));ipcMain.handle('choose-project',chooseProject);ipcMain.handle('runtime-status',async()=>({backend:await waitHttp(`http://127.0.0.1:${PORT}/health`,1000),worker:await waitHttp(`http://127.0.0.1:${WORKER_PORT}/health`,1000),projectRoot}));
async function createWindow(){windowRef=new BrowserWindow({width:1280,height:820,minWidth:900,minHeight:600,title:'JARVIS',backgroundColor:'#05070b',webPreferences:{preload:path.join(DESKTOP_DIR,'preload.cjs'),contextIsolation:true,nodeIntegration:false,sandbox:true}});await windowRef.loadURL(`http://127.0.0.1:${PORT}`);windowRef.webContents.setWindowOpenHandler(({url})=>{shell.openExternal(url);return{action:'deny'}})}
app.whenReady().then(async()=>{await loadProject();try{await ensureRuntime()}catch(e){console.error(e.message)}await createWindow();if(!projectRoot)await dialog.showMessageBox(windowRef,{type:'warning',title:'JARVIS',message:'Projekt nebyl automaticky nalezen. Připoj ho z JARVIS nastavení.'})});app.on('before-quit',stopRuntime);app.on('window-all-closed',()=>{if(process.platform!=='darwin')app.quit()});
