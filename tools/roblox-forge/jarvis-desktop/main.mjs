...
async function ensureRuntime(){
  ...
  if(!backend||backend.exitCode!==undefined){
    // Safe first-launch defaults: one local model, 3B, no autonomous worker.
    backend=startNode(path.join(projectRoot,'tools','roblox-forge','jarvis-backend-v2.mjs'),{
      JARVIS_MODEL:process.env.JARVIS_MODEL||'qwen2.5-coder:3b',
      JARVIS_AUTONOMY:'0',
    });
  }
  ...
  // Do not auto-start the worker. On a 4-core CPU it can load another local model
  // while the conversational backend is already running and cause system stalls.
  const workerOk=Boolean(worker&&worker.exitCode===undefined)&&await waitHttp(`http://127.0.0.1:${WORKER_PORT}/health`,1000);
