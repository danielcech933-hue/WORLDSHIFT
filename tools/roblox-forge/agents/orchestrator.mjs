...
async function discoverAgents() {
  ...
  return Promise.all(registry.agents.map(async agent => {
    const executable = await commandExists(agent.command);
    return { ...agent, executable, available: Boolean(executable), running: [...running.values()].some(x => x.agent === agent.id) };
  }));
}

function json(res, status, body) {
  res.writeHead(status, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store', 'Access-Control-Allow-Origin': 'http://127.0.0.1:43117' });
...
async function runAgent({ role = 'coder', prompt, agentId, policy = DEFAULT_POLICY }) {
  ...
  const registry = await readRegistry();
  const agents = await Promise.all(registry.agents.map(async agent => {
    const executable = await commandExists(agent.command);
    return { ...agent, executable, available: Boolean(executable) };
  }));
  ...
  child.stdout?.on('data', chunk => { stdout = append(stdout, chunk, 200000); });
  child.stderr?.on('data', chunk => { stderr = append(stderr, chunk, 100000); });

  return await new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      try { child.kill(); } catch {}
      running.delete(id);
      resolve({ ok: false, id, agent: agent.id, role, code: null, signal: 'TIMEOUT', stdout, stderr, error: 'Agent timed out.' });
    }, RUN_TIMEOUT_MS);
    child.on('error', error => { clearTimeout(timer); running.delete(id); reject(error); });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') return json(res, 200, { ok: true, port: PORT, policy: DEFAULT_POLICY, agents: await discoverAgents(), running: [...running.values()].map(({ child, ...item }) => item) });
    ...
    if (req.method === 'POST' && url.pathname === '/stop') {
      ...
      return json(res, 200, { ok: true, stopped: data.id });
    }
