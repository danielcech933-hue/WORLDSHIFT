...
function spawnAgent(executable,args,pipeStdin=false){
    const stdio = [pipeStdin ? 'pipe' : 'ignore', 'pipe', 'pipe'];
    if(process.platform !== 'win32' || !/\.(cmd|bat)$/i.test(executable)) {
        return spawn(executable,args,{cwd:PROJECT_ROOT,env:{...process.env},windowsHide:true,shell:false,stdio});
    }
    // Windows .cmd launchers are invoked through cmd.exe, while the actual prompt
    // is supplied over stdin for Codex so spaces/Unicode can never split argv.
    return spawn('cmd.exe',['/d','/c','call',executable,...args],{cwd:PROJECT_ROOT,env:{...process.env},windowsHide:true,shell:false,stdio});
}
function buildArgs(agent,prompt,role,policy){
    const roleHint=`You are the ${role} agent inside ROBLOX FORGE. Work in the connected project at ${PROJECT_ROOT}. ${prompt}`;
    const args=[...(agent.run?.argsPrefix||[])];
    if(agent.id==='opencode'){args.push('--agent',role==='reviewer'?'plan':'build');if(policy==='auto')args.push('--auto');}
    if(agent.id==='codex'){
        // Codex supports `codex exec -` to read the prompt from stdin.
        args.push('-');
        return {args,prompt:roleHint};
    }
    args.push(roleHint);
...
async function runAgent({role='coder',prompt,agentId,policy=DEFAULT_POLICY}){
    ...
    if(!['architect','coder','reviewer','researcher','tester'].includes(role))throw new Error(`Unsupported role '${role}'.`);
    const registry=await readRegistry();
    const agents=await Promise.all(registry.agents.map(async agent=>{const executable=await commandExists(agent.command);return {...agent,executable,available:Boolean(executable)};}));
    ...
    const built=buildArgs(agent,prompt,role,policy);
    const child=spawnAgent(agent.executable,built.args,Boolean(built.prompt));
    running.set(id,{id,agent:agent.id,role,pid:child.pid||null,startedAt:new Date().toISOString(),child});
    let stdout='',stderr='';
    const append=(target,chunk,limit)=>(target+String(chunk)).slice(-limit);
    child.stdout?.on('data',c=>stdout=append(stdout,c,200000));
    child.stderr?.on('data',c=>stderr=append(stderr,c,100000));
    if(built.prompt && child.stdin){
        child.stdin.on('error',()=>{});
        child.stdin.end(built.prompt);
    }
    return await new Promise((resolve,reject)=>{
        const timer=setTimeout(()=>{try{child.kill();}catch{}running.delete(id);resolve({ok:false,id,agent:agent.id,role,code:null,signal:'TIMEOUT',stdout,stderr,error:'Agent timed out.'});},RUN_TIMEOUT_MS);
        child.on('error',e=>{clearTimeout(timer);running.delete(id);reject(e);});
        child.on('close',(code,signal)=>{clearTimeout(timer);running.delete(id);resolve({ok:code===0,id,agent:agent.id,role,code,signal,stdout,stderr});});
    });
}
