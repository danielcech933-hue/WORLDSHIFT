...
function quoteWindowsArg(value){const text=String(value);if(!/[\s"&|<>^]/.test(text))return text;return `"${text.replace(/(\\*)"/g,'$1$1\\"').replace(/(\\+)$/g,'$1$1')}"`;}
function spawnAgent(executable,args){if(process.platform!=='win32'||!/\.(cmd|bat)$/i.test(executable))return spawn(executable,args,{cwd:PROJECT_ROOT,env:{...process.env},windowsHide:true,shell:false,stdio:['ignore','pipe','pipe']});const commandLine=[executable,...args].map(quoteWindowsArg).join(' ');return spawn('cmd.exe',['/d','/s','/c',commandLine],{cwd:PROJECT_ROOT,env:{...process.env},windowsHide:true,shell:false,stdio:['ignore','pipe','pipe']});}
